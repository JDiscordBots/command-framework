...
public interface CommandEvent
{
	...
	/**
	 * Sends a message in response to the invoked command.
	 * @param message information about the message to send as {@link MessageCreateData}
	 * @return a {@link RestAction} that can be used to actually send and react to sending this message
	 */
	RestAction<Message> reply(MessageCreateData message);
	...
	{
		MessageCreateBuilder createDataBuilder = new MessageCreateBuilder()
				.setContent(message)
				.setEmbeds(embed);
		for (ActionRow actionRow : actionRows){
			createDataBuilder.addActionRow(actionRow.getComponents());
		}
		return reply(createDataBuilder.build());
	}
